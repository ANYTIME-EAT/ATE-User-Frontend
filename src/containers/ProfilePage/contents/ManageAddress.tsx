...
const ManageAddress: FC<ManageAddressProps> = ({ className = "" }) => {
  ...
  const getAllAddressData = async () => {
    ...
    if (response.data) {
      if (response.data.data.length > 0) {
        let addressArr = response.data.data[0].address;
        let tempData: any = [];
  ...
  const removeAddressHandler = async () => {
    const newAddressList = userAddress.filter((item: any) => 
       item.addressType !== selectedAddressType
    );
  ...
  const updateAddress = async () => {
    userAddress.map((item: any, key: number) => {
      if (item.addressType === selectedAddressType) {
        userAddress[key] = {
